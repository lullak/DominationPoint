...
namespace DominationPointTests.UnitTests.Services.Hosted
{
 ...
 new GameParticipant { GameId =1, ApplicationUserId = "user1", ApplicationUser = new ApplicationUser { UserName = "Team1", ColorHex = "#000000" } }
        ...
        public async Task ExecuteAsync_CompletesSuccessfully()
        {
            ...
            var games = new List<Game> { new Game { Id = 1, Name = "TestGame", Status = GameStatus.Active } };
            var mockGamesSet = MockDbSetHelper.GetMockDbSet(games.AsQueryable());
            mockContext.Setup(c => c.Games).Returns(mockGamesSet.Object);
            mockScoreboard.Setup(s => s.CalculateScoreboardAsync(1))
                .ReturnsAsync(new ScoreboardViewModel { Game = games[0], TeamScores = new List<TeamScore>() });
            ...
            // ACT
            await service.StartAsync(CancellationToken.None);
            await Task.Delay(100); // Give timer time to execute
            await service.StopAsync(CancellationToken.None);

            // ASSERT
            Assert.Contains(logger.Invocations, inv =>
                inv.Method.Name == "Log" &&
                inv.Arguments.Count > 2 &&
                inv.Arguments[2]?.ToString()?.Contains("Live Score Update Service is running.") == true);
        }


        [Fact]
        public void DoWork_ParticipantNotFound_SkipsScore()
        {
            // ARRANGE
            var logger = new Mock<ILogger<LiveScoreUpdateService>>();
            var mockContext = new Mock<IApplicationDbContext>();
            ...
            // Scoreboard returns a team that is NOT in participants
            mockScoreboard.Setup(s => s.CalculateScoreboardAsync(1))
                .ReturnsAsync(new ScoreboardViewModel
                {
                    Game = games[0],
                    TeamScores = new List<TeamScore> {
                new TeamScore { TeamName = "NonExistentTeam", HoldingScore = 100 }
                    }
                });
            ...
            // ACT
            service.GetType().GetMethod("DoWork", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)!
                .Invoke(service, new object[] { null });

            // ASSERT
            mockScoresSet.Verify(s => s.Add(It.IsAny<GameScore>()), Times.Never); // Should not add score
            Assert.Contains(logger.Invocations, inv =>
                inv.Method.Name == "Log" &&
                inv.Arguments.Count > 2 &&
                inv.Arguments[2]?.ToString()?.Contains("Successfully updated live scores") == true);
        }

        [Fact]
        public void DoWork_ParticipantFound_AddsScore()
        {
            // ARRANGE
            var logger = new Mock<ILogger<LiveScoreUpdateService>>();
            var mockContext = new Mock<IApplicationDbContext>();
            ...
            var participants = new List<GameParticipant> {
        new GameParticipant { GameId = 1, ApplicationUserId = "user1", ApplicationUser = user }
    };
            ...
            mockScoreboard.Setup(s => s.CalculateScoreboardAsync(1))
                .ReturnsAsync(new ScoreboardViewModel
                {
                    Game = games[0],
                    TeamScores = new List<TeamScore> {
                new TeamScore { TeamName = "Team Red", TotalScoreFromDb = 500 }
                    }
                });
            ...
            // ASSERT
            mockScoresSet.Verify(s => s.Add(It.Is<GameScore>(gs =>
                gs.GameId == 1 &&
                gs.ApplicationUserId == "user1" &&
        ...
        [Fact]
        public void DoWork_MixedParticipants_AddsOnlyFoundTeams()
        {
            // ARRANGE
            var logger = new Mock<ILogger<LiveScoreUpdateService>>();
            var mockContext = new Mock<IApplicationDbContext>();
            ...
            var userRed = new ApplicationUser { Id = "red", UserName = "Team Red", ColorHex = "#FF0000" };
            var participants = new List<GameParticipant> {
        new GameParticipant { GameId = 1, ApplicationUserId = "red", ApplicationUser = userRed } 
        // Team Blue is NOT in participants
    };
            ...
            mockScoreboard.Setup(s => s.CalculateScoreboardAsync(1))
                .ReturnsAsync(new ScoreboardViewModel
                {
                    Game = games[0],
                    TeamScores = new List<TeamScore> {
                new TeamScore { TeamName = "Team Red", TotalScoreFromDb = 300 },
                new TeamScore { TeamName = "Team Blue", TotalScoreFromDb = 200 } // Not found
                    }
                });
            ...
            // ASSERT
            mockScoresSet.Verify(s => s.Add(It.Is<GameScore>(gs => gs.ApplicationUserId == "red")), Times.Once);
            mockScoresSet.Verify(s => s.Add(It.IsAny<GameScore>()), Times.Once); // Only one score added
        }

        [Fact]
        public void DoWork_OldScoresExist_RemovesBeforeAdding()
        {
            // ARRANGE
            var logger = new Mock<ILogger<LiveScoreUpdateService>>();
            var mockContext = new Mock<IApplicationDbContext>();
        ...
        new GameParticipant { GameId = 1, ApplicationUserId = "user1", ApplicationUser = user }
            ...
            // Old scores exist
            var oldScores = new List<GameScore> {
        new GameScore { Id = 99, GameId = 1, ApplicationUserId = "user1", Points = 100 }
    };
            ...
            var mockScoresSet = MockDbSetHelper.GetMockDbSet(oldScores.AsQueryable());
            mockContext.Setup(c => c.GameScores).Returns(mockScoresSet.Object);
            ...
            // ASSERT
            mockScoresSet.Verify(s => s.RemoveRange(It.IsAny<IEnumerable<GameScore>>()), Times.Once);
            mockScoresSet.Verify(s => s.Add(It.IsAny<GameScore>()), Times.Once);
        }

        [Fact]
        public void DoWork_CalculateScoreboardThrows_LogsError()
        {
            // ARRANGE
            var logger = new Mock<ILogger<LiveScoreUpdateService>>();
            var mockContext = new Mock<IApplicationDbContext>();
            ...
            // CalculateScoreboardAsync throws
            mockScoreboard.Setup(s => s.CalculateScoreboardAsync(1))
            ...
            Assert.Contains(logger.Invocations, inv =>
                inv.Method.Name == "Log" &&
                inv.Arguments.Count > 2 &&
                inv.Arguments[2]?.ToString()?.Contains("Error occurred while updating live scores.") == true);
        }
